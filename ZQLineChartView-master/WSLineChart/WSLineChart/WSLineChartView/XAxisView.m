...
- (void)drawRect:(CGRect)rect {
    ...
    ///
    [self drawLine:context startPoint:CGPointMake(0, 0) endPoint:CGPointMake(self.frame.size.width, 0) lineColor:WSChartLineGrayColor lineWidth:1 isArc:NO];
    ////////////////////// X轴文字 //////////////////////////
    // 添加坐标轴Label
    for (int i = 0; i < self.xTitleArray.count; i++) {
        ...
        if (i == 0) {
            ...
            [title drawInRect:titleRect withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:WSChartTextColor}];
            ...
            [self drawLine:context
                startPoint:CGPointMake(titleRect.origin.x+labelSize.width/2, self.frame.size.height - labelSize.height-5)
                 ...
                 lineColor:WSChartLineGrayColor
                 lineWidth:0.6
                     isArc:NO];
        }
        // 如果Label的文字有重叠，那么不绘制
    ...
    if (self.isShowHorLine) {
        CGFloat separateMargin = (self.frame.size.height  - WSAxisTextGap - textSize.height - topMargin  - 5 * 1) / WSNumberOfYAxisElements;
        for (int i = 0; i < WSNumberOfYAxisElements; i++) {
    ...
    if (self.yValueArrays && self.yValueArrays.count > 0){
        NSArray *chartLineColorArr = @[WSChartLineBlueColor,WSChartLineOrangeColor,WSChartLineGreenColor];
...
- (void)drawDataLineContext:(CGContextRef)context yValueArray:(NSArray *)yValueArray textSize:(CGSize)textSize lineColor:(UIColor *)lineColor{
    ...
    for (NSInteger i = 0; i < yValueArray.count; i++) {
        ...
        if (i == yValueArray.count-1) {
            ...
            CGFloat chartHeight = self.frame.size.height - textSize.height - WSAxisTextGap - topMargin;
            ...
            if (_isShowLabel) {
                ...
                if (i != 0) {
                    if ((maxX + 3) > strRect.origin.x) {
                        ...
                        [str drawInRect:strRect withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:WSChartTextColor}];
...
//        NSString *num = [cityValues objectAtIndex:nowPoint];
        ...
        //画选中的红线
        [self drawLine:context startPoint:CGPointMake(selectPoint.x, 0) endPoint:CGPointMake(selectPoint.x, self.frame.size.height- textSize.height - 5) lineColor:WSChartLineGedColor lineWidth:1 isArc:NO];
