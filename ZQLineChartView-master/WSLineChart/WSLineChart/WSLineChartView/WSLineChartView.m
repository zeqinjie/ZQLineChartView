...
- (void)creatLine{
    ...
    bottomLine.backgroundColor = WSChartLineGrayColor;
    [self addSubview:bottomLine];
    
//    UIView *topLine = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 0.6)];
//    topLine.backgroundColor = WSChartLineGrayColor;
//    [self addSubview:topLine];
}

- (void)creatYAxisView {
    if (!self.yAxisView) {
        self.yAxisView = [[YAxisView alloc]initWithFrame:CGRectMake(self.frame.size.width - WSLeftMargin ,0, WSLeftMargin, self.frame.size.height)];
...
- (void)creatXAxisView {
    if (!self.xAxisView) {
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width - WSLeftMargin, self.frame.size.height)];
...
- (void)reloadData:(NSArray*)xTitleArray
    ...
    self.xAxisView.frame = CGRectMake(0, 0, self.xTitleArray.count * self.pointGap + WSLastSpace, self.frame.size.height);
    ...
    if (animation) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:1.0 animations:^{
                [weakSelf.scrollView scrollRectToVisible:CGRectMake(0, 0, self.xTitleArray.count * self.pointGap + WSLastSpace, self.frame.size.height) animated:NO];
            }];
        });
    }
...
- (void)pinchGesture:(UIPinchGestureRecognizer *)recognizer
{
    if (recognizer.state == 3) {
        ...
        if (self.xAxisView.frame.size.width-WSLastSpace <= self.scrollView.frame.size.width) { //当缩小到小于屏幕宽时，松开回复屏幕宽度
            
            CGFloat scale = self.scrollView.frame.size.width / (self.xAxisView.frame.size.width-WSLastSpace);
            ...
            [UIView animateWithDuration:0.25 animations:^{
                ...
                frame.size.width = self.scrollView.frame.size.width+WSLastSpace;
                self.xAxisView.frame = frame;
            }];
            
            self.xAxisView.pointGap = self.pointGap;
            
        }else if (self.xAxisView.frame.size.width-WSLastSpace >= self.xTitleArray.count * _defaultSpace){
            
            [UIView animateWithDuration:0.25 animations:^{
                CGRect frame = self.xAxisView.frame;
                frame.size.width = self.xTitleArray.count * _defaultSpace + WSLastSpace;
                self.xAxisView.frame = frame;
