...
- (void)drawRect:(CGRect)rect {
    ...
    // Label做占据的高度
    CGFloat allLabelHeight = self.frame.size.height - WSAxisTextGap - labelSize.height ;
    // Label之间的间隙
//    CGFloat labelMargin = (allLabelHeight + labelSize.height - (numberOfYAxisElements + 1) * labelSize.height) / numberOfYAxisElements;
    CGFloat labelMargin = (allLabelHeight - topMargin )/WSNumberOfYAxisElements;
    [self drawLine:context startPoint:CGPointMake(self.frame.size.width-WSLeftMargin, 0) endPoint:CGPointMake(self.frame.size.width-WSLeftMargin, allLabelHeight) lineColor:WSChartLineGrayColor lineWidth:1];
    ...
    CGFloat wordOrginX = self.frame.size.width-WSLeftMargin;//self.frame.size.width - 1-5
    ...
    // 添加Label
    for (int i = 0; i < WSNumberOfYAxisElements + 1; i++) {

        CGFloat avgValue = (self.yMax - self.yMin) / WSNumberOfYAxisElements;
//        // 判断是不是小数
